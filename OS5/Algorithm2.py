...
class Algorithm2(Algorithms):
    def __init__(self, processors, max_no_searches):
        super().__init__(processors)
        self.max_no_searches = max_no_searches

    def get_processor(self, process):
        # Check if process should be executed on the original processor
        if (pr := self.processors[process.no_processor]).should_process_be_executed():
            return pr
        ...
        # Brute force check if any processor could perform this operation
