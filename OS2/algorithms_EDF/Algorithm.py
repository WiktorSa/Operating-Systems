...
class Algorithm(ABC):
    ...
    def get_argmin_distance_r_requests(wait_r_requests: np.array, disc: Disc):
        min_index = 0
        for i in range(1, len(wait_r_requests)):
            if abs(disc.current_position - wait_r_requests[min_index].block_position) > \
                    abs(disc.current_position - wait_r_requests[i].block_position):
                min_index = i
