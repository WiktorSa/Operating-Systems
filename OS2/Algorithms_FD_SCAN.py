...
# This works
def FCFS_FD_SCAN(requests, real_time_requests, disc):
    ...
    while True:
        ...
        # Processing all the real time requests (FD_SCAN algorithm)
        while len(waiting_real_time_requests) > 0 or current_request.is_real_time:
            ...
            if not current_request.is_real_time:
                ...
                # Only taking feasible real time requests into consideration
                while len(waiting_real_time_requests) > 0 and abs(
                        current_request.block_position - disc.current_position) > current_request.deadline_time:
                    # We count every unrealised real time process as finished
                    no_of_rejected_real_time_requests += 1
                    finished_requests.append(current_request)
                    current_request = waiting_real_time_requests.pop(0)

                # Even the last real time process isn't feasible
                if abs(current_request.block_position - disc.current_position) > current_request.deadline_time:
                    no_of_rejected_real_time_requests += 1
                    finished_requests.append(current_request)
            ...
            # Checking if any processes are in the way and could be finished
            for request in waiting_requests:
                if request.block_position == disc.current_position:
                    finished_requests.append(request)
                    waiting_requests.remove(request)

            for request in waiting_real_time_requests:
                if request.block_position == disc.current_position:
                    finished_requests.append(request)
                    waiting_real_time_requests.remove(request)
...
# This works
def SSTF_FD_SCAN(requests, real_time_requests, disc):
    ...
    while True:
        ...
        # Processing all the real time requests (FD-SCAN algorithm)
        while len(waiting_real_time_requests) > 0 or current_request.is_real_time:
            # Swap the current process if it's not real time
            if not current_request.is_real_time:
                waiting_requests.insert(0, current_request)
    ...
    # Here we display last 15 requests that have been finished
    # (Note to yourself - for testing purposes we only show the last 5 requests
    print("SSTF-FD-SCAN last 15 finished requests")
    for request in list(reversed(finished_requests))[:15]:
        print(request)
    finished_requests.sort(key=lambda x: x.waiting_time)
    print("Longest waiting request: {longest_request}".format(longest_request=finished_requests[-1]))
...
# SCAN
# This works
def SCAN_FD_SCAN(requests, real_time_requests, disc, is_going_left):
    # Sorting by arrival time and by how
    requests.sort(key=lambda x: x.arrival_time)
    real_time_requests.sort(key=lambda x: x.arrival_time)
    ...
    # Number of reject requests
    no_of_rejected_real_time_requests = 0
    # This request is fake. It's only used to make sure that FD-SCAN is working properly
    ...
    # Here we display last 15 requests that have been finished
    # (Note to yourself - for testing purposes we only show the last 5 requests
    print("SCAN-FD-SCAN last 15 finished requests")
    for request in list(reversed(finished_requests))[:15]:
        print(request)
    finished_requests.sort(key=lambda x: x.waiting_time)
    print("Longest waiting request: {longest_request}".format(longest_request=finished_requests[-1]))
...
# This works
def CSCAN_FD_SCAN(requests, real_time_requests, disc, is_going_left):
    # Sorting by arrival time and by how
    requests.sort(key=lambda x: x.arrival_time)
    real_time_requests.sort(key=lambda x: x.arrival_time)
    ...
    # Here we display last 15 requests that have been finished
    # (Note to yourself - for testing purposes we only show the last 5 requests
    print("C-SCAN-FD-SCAN last 15 finished requests")
    for request in list(reversed(finished_requests))[:15]:
        print(request)
    finished_requests.sort(key=lambda x: x.waiting_time)
    print("Longest waiting request: {longest_request}".format(longest_request=finished_requests[-1]))
